    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4">
          ...
          {/* Modal Content */}
          <div className="bg-white/80 backdrop-blur-2xl rounded-[2rem] shadow-[0_16px_64px_rgba(0,0,0,0.1)] border border-white/80 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            
            {/* Header */}
            <div className="flex justify-between items-center p-6 md:px-8 border-b border-white/40 bg-gradient-to-r from-white/40 to-transparent shrink-0">
              <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600">
                Add New Category
              </h2>
              <button
                onClick={() => setIsOpen(false)}
                className="text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl p-2 transition-all duration-200"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto p-6 md:px-8 custom-scrollbar">
              <form id="add-category-form" onSubmit={handleSubmit} className="space-y-6">
                
                {/* Error Message */}
                {errorMessage && (
                  <div className="p-4 rounded-2xl bg-rose-50/80 backdrop-blur-sm border border-rose-100/50 flex items-start space-x-3 shadow-sm animate-in fade-in slide-in-from-top-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, slug: e.target.value })}
                    placeholder="e.g. web-development"
                    className="w-full px-4 py-3 rounded-xl bg-white/30 backdrop-blur-md border border-white/60 focus:bg-white focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 outline-none transition-all duration-300 shadow-[0_2px_10px_rgba(0,0,0,0.02)] text-slate-800 placeholder-slate-400"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    onChange={handleBannerChange}
                    className="w-full px-4 py-3 rounded-xl bg-white/50 backdrop-blur-md border border-white/60 focus:bg-white focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 outline-none transition-all duration-300 shadow-[0_2px_10px_rgba(0,0,0,0.02)] text-slate-800
                    file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer"
                  />
                  ...
                  {bannerPreview && (
                    <div className="relative mt-4 h-48 w-full overflow-hidden rounded-2xl border border-white/80 bg-slate-100/50 shadow-sm animate-in fade-in zoom-in">
                      <Image
                        src={bannerPreview}
                        alt="Banner preview"
                        fill
            ...
            {/* Footer Actions */}
            <div className="flex items-center justify-end gap-3 p-6 md:px-8 border-t border-white/40 bg-white/20 shrink-0">
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="px-6 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-900 bg-white/50 hover:bg-white/80 border border-transparent hover:border-white rounded-xl transition-all duration-300 backdrop-blur-sm"
              >
                Cancel
              </button>
              <button
